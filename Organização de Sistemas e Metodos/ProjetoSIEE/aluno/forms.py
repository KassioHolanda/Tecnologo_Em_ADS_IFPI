...
class RegisterMyCurriculum(forms.ModelForm):
    experiencia_profissional = forms.CharField(max_length=255,
...
class ContactCompanyForm(forms.Form):
    ...
    message = forms.CharField(label='Mensagem', max_length=100,
                              widget=forms.Textarea(attrs={'class': 'materialize-textarea'}),
                              required=False)
    # file = forms.FileField(label='Anexe seu Curriculo', widget=forms.FileInput(attrs={'class': 'file-field input-field'}),required=False)
    # attach = forms.Field(label=u'Anexar currículo', widget=forms.FileInput, required=False)
